...
app.controller('typeTemplateController', function ($scope, $controller, baseService) {
    ...
    $scope.findBrandList = function () {
        baseService.sendGet("/brand/fingBrandList").then(function (response) {
            /*定义select2的数据源（格式是固定的）*/
            //{}产生option value - id test - text
    ...
    $scope.findSpecList = function () {
        baseService.sendGet("/specification/fingBrandList").then(function (response) {
            /*定义select2的数据源（格式是固定的）*/
            //{}产生option value - id test - text
